...
import           Beam.Types (Db(..), EntityT(..), createTableMigration, db, dropTableMigration)
import           Generators
import           Types
import           Typson
import           Typson.Beam
...
beamTestTree = withRunDb $ \runDb ->
  testCase "JSON Queries" $ do
    ...
    let a1 = flip map graphs $ \g -> B.PgJSONB $
    ...
    let a2 = flip map graphs $ \g -> B.PgJSONB $
          g ^. fieldLens (key @"baz1") bazJ
    ...
    let a3 = flip map graphs $ \g -> B.PgJSONB $
          g ^. fieldLens (key @"baz1") bazJ
    ...
    let a4 = flip map graphs $ \g -> B.PgJSONB $
          g ^? fieldLens (key @"baz1") bazJ
    ...
    let a5 = flip map graphs $ \g -> B.PgJSONB $
          g ^? fieldLens (key @"baz1") bazJ
             . fieldLens (key @"bar2") barJ
             . _Just
    ...
    let a6 = flip map graphs $ \g -> B.PgJSONB $
    ...
    let a7 = flip map graphs $ \g -> B.PgJSONB $
          g ^? fieldLens (key @"baz1") bazJ
             . fieldLens (key @"bar1") barJ
    ...
    let a8 = flip map graphs $ \g -> B.PgJSONB $
    ...
    let a9 = flip map graphs $ \g -> B.PgJSONB $
          g ^? fieldLens (key @"baz3") bazJ
             . ix 0
             . fieldLens (key @"foo1") fooJ
             . ix 1
...
getAllGraphs :: B.Q B.Postgres Db s (B.QGenExpr B.QValueContext B.Postgres s (B.PgJSONB Baz))
...
connectToDb = do
  ...
  _ <- B.runBeamPostgres conn $
    B.executeMigration B.runNoReturn $ do
      dropTableMigration
...
insertData graphs =
  let mkEntity g = EntityT { _entityId = B.default_
                           , _entityGraph = B.val_ (B.PgJSONB g)
                           }
   in B.runInsert
